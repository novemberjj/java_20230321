...
public class ThreadEx01 {
	...
	public static void main(String[] args) {
		/* Thread(쓰레드) 
		 * - 실행중인 프로그램이 OS(운영체제)로부터 
		 * 메모리를 할당 받아 process 상태가 됨
		 ...
		 * */
		System.out.println(Thread.currentThread()); //현재 실행 중인 thread
		
		MyThread th = new MyThread();
		th.start(); //start() -> run() 호출
		
		MyThread th2 = new MyThread();
		th2.start(); //하나의 클래스에 두 개의 thread 
