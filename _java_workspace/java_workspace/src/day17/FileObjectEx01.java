...
public class FileObjectEx01 {
	...
	public static void main(String[] args) {
		...
		// fstr에서 파일경로만 추출 출력
		System.out.println("파일경로:"+fstr.substring(fstr.indexOf(File.separator)+1, fstr.lastIndexOf(File.separator))); // "\" ->하나만 인식 못함 
		
		//파일명과 확장자 분리
		System.out.println(f.getName()); //파일명.확장자
		String[] str= f.getName().split(File.separator+"."); //"."혼자 쓸 수 없음
		System.out.println("파일명:"+str[0]+", 확장자:"+str[1]);
		
		//StringTokenizer 클래스 사용
		StringTokenizer stk = new StringTokenizer(f.getName(),".");
		while(stk.hasMoreElements()) { //토큰요소의 값이 있다면 true, false
